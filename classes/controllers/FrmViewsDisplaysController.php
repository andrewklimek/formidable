...
class FrmViewsDisplaysController {
	 ...
	 * @return bool
	 */
	public static function replace_editor( $editor, $the_post ) {
		if ( 'frm_display' === $the_post->post_type ) {
			self::maybe_update_post_content( $the_post );
			self::maybe_update_dyncontent( $the_post );
		}
		return $editor;
	}
	 ...
	 * @param WP_Post $post
	 */
	private static function maybe_update_dyncontent( $post ) {
		$dyn_content = get_post_meta( $post->ID, 'frm_dyncontent', true );
		if ( ! $dyn_content ) {
			return;
		}

		$content = maybe_unserialize( $dyn_content );
		if ( ! FrmViewsAppHelper::unserialized_content_is_grid_format( $content ) ) {
			return;
		}

		$helper = new FrmViewsLayoutHelper( $post );
		update_post_meta( $post->ID, 'frm_dyncontent', $helper->flatten( $content, 'detail' ) );
	}
